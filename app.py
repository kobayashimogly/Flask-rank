    ...
    if file:
        try:
            ...
            with open(filepath, newline='', encoding='utf-8') as csvfile:
                ...
                for row in reader:
                    if len(row) < 2:
                        logging.warning(f"Row {row} is malformed. It should have at least two columns (keyword, article_id).")
                        continue
                    # 全角スペースを半角スペースに変換して処理
                    keyword = row[0].replace('\u3000', ' ').strip()  # 全角スペースを半角スペースに変換してトリム
                    article_id = row[1].strip()  # 記事IDをトリム
            ...
            # 完了メッセージとダウンロードボタンの表示
            return f'''
                <h3>実行が完了しました！</h3>
                <form action="/download_csv" method="get">
                    <input type="submit" value="結果をダウンロード">
                </form>
            '''
